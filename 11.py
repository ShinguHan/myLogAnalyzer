...
def check_missing_in_log(standard_info, log_info):
    missing_messages = []

    # Convert the log info to a set for faster lookup
    log_messages = set(
        (p1, p2, m)
        for time, direction, m, detail in log_info
        for p1, p2 in [("cv", "mc") if direction == "->>" else ("mc", "cv")]
    )

    # Check each standard message
    for p1, p2, message, ceid in standard_info:
        if (p1, p2, message) not in log_messages:
            missing_messages.append((p1, p2, message))
...
missing_messages = check_missing_in_log(standard_info, log_info)
